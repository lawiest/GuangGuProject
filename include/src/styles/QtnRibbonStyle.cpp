    ...
    if (const StyleOptionBackstageButton* toolbutton = qstyleoption_cast<const StyleOptionBackstageButton*>(opt))
    {
        ...
        else
        {
            ...
            if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
            {
                ...
                {
                    int heightTotal = pmSize.height();

                    alignment |= Qt::AlignHCenter | Qt::TextWordWrap;
                    const int space = toolbutton->fontMetrics.width(QLatin1Char('X'));
                    QRect rcText(QStyle::visualRect(opt->direction, rect, tr));
                    QRect rcRowText(rcText);
                    rcRowText.adjust(space, 0, -space, 0);

                    QSize sz = toolbutton->fontMetrics.boundingRect(rcRowText, alignment, toolbutton->text).size();
                    heightTotal += sz.height() + 2;

                    pr.setY((opt->rect.height() - heightTotal)/2);

                    if (!hasArrow)
                        drawItemPixmap(p, pr, Qt::AlignTop|Qt::AlignHCenter, pm);

                    rcRowText.adjust(-space, 0, space, 0);
                    rcRowText.setTop(pr.top() + pmSize.height() + 4);

                    rcRowText.adjust(6, 0, -6, 0);
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        else if (toolbutton->features & QStyleOptionToolButton::HasMenu) 
        {
            ...
            if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon)
            {
                ...
                pr.setY((opt->rect.height() - heightTotal) / 2);
                rcRowText.setTop(pr.top() + pmSize.height() + 4);
                rcRowText.setSize(sz);
                newBtn.rect = QRect(QPoint(rcRowText.right() + space, rcRowText.bottom() - toolbutton->fontMetrics.height()/2), QSize(5, 5));
            }
            else
            {
                QRect ir = menuarea;
