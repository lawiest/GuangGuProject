...
namespace GeoGlobe
{
	...
	osg::Group* CObliqueImageLoader::LoadFromFile(std::string path)
	{
	...
	osg::Group* CObliqueImageLoader::LoadFromServer(std::string strUrl)
	{
		osg::Group* pOImageGroup = new osg::Group();
		...
		std::fstream f("./osgbIndex.txt", ios::out);
		...
		for(int i=0;i<pathFiles.size()-1;i++)
		{
			...
			if (f.is_open())
			{
				f << bs.center().x() << "," << bs.center().y() << "," << bs.center().z() << " " << httpPath << "\n";
			}
