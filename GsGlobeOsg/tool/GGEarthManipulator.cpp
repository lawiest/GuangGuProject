...
using namespace KERNEL_NAME::Globe;
...
EarthManipulator::Settings::getAction(int event_type, int input_mask, int modkey_mask) const
{
	...
	//中键旋转和右键互换
	if (event_type == 8 && input_mask== 4)
	{
		input_mask = 2;
	}
	else if (event_type == 8 && input_mask == 2)
	{
		input_mask = 4;
	}
...
EarthManipulator::established()
{
    ...
    if ( !_homeViewpoint.isSet() )
    {
        ...
        {
            Viewpoint vp;
            vp.focalPoint() = GeoPoint(_srs.get(), 110.0, 30, 0, ALTMODE_ABSOLUTE);
            vp.heading()->set( 0.0, Units::DEGREES );
            vp.pitch()->set( -90, Units::DEGREES );
            vp.range()->set( _srs->getEllipsoid()->getRadiusEquator() * 1.7, Units::METERS );
...
void EarthManipulator::setSmooth(const bool& bSmooth)
{
	_bIsSmooth = bSmooth;
}

bool EarthManipulator::getSmooth()
{
	return _bIsSmooth;
}

void EarthManipulator::setFixedAngle(const bool& bFixedAngle)
{
	_bIsFixedAngle = bFixedAngle;
}

bool EarthManipulator::getFixedAngle()
{
	return _bIsFixedAngle;
}
    ...
    if ( !handled )
    {
        ...
        switch( ea.getEventType() )
        {
            case osgGA::GUIEventAdapter::PUSH:
				...
				//打断数据定位，用1ms的设置当前位置来打断。
				here = getViewpoint();
				if (!here.focalPoint()->getSRS())
					return false;
				setViewpoint(here, 0.001);
            ...
            case osgGA::GUIEventAdapter::DRAG:
                {
					...
					if (!_bIsFixedAngle || action._type == EarthManipulator::ACTION_PAN)
					{    
...
void
EarthManipulator::rotateByZ(double dDelta)
 {
	_settings->setLockAzimuthWhilePanning(true);
	_centerRotation = computeCenterRotation(_center);
	 
	getEulerAnglesByZ(_rotation, 0L, &dDelta);
	...
	osg::Quat rotate_azim;
	rotate_azim.makeRotate(dDelta, localUp);

	_rotation = _rotation * rotate_azim;
}

void
EarthManipulator::getEulerAnglesByZ(const osg::Quat& q, double* out_azim, double* out_pitch) const
{
	osg::Matrix m(q);
...
const Viewpoint& EarthManipulator::getHomeViewpoint()
{
	return _homeViewpoint.get();
}
