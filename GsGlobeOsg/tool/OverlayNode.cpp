...
OverlayNode::OverlayNode(OverlayTechnique technique):
	...
	m_vecViewDirection(osg::Vec3d(0.0, 0.0, 0.0))
{
...
OverlayNode::OverlayData* OverlayNode::getOverlayData(osgUtil::CullVisitor* cv)
{
    ...
    if (!overlayData->_y0) overlayData->_y0 = new osg::Uniform("y0",-0.5f);
...
void OverlayNode::setProjectDirection(osg::Vec3d viewDirection)
{
        ...
        if (_continuousUpdate || _updateCamera)
        {
            ...
            if (bs.valid())
            {
                ...
                else
                {
                    ...
                    osg::Vec3d viewDirection(1.0,-0.8,0.0);

                    double viewDistance = bs.radius()/*2.0*bs.radius()*/;
                    osg::Vec3d center = bs.center();
					
					viewDirection = m_vecViewDirection;

					if (m_vecViewDirection == osg::Vec3d(0.0,0.0,0.0))
...
void OverlayNode::removeOverlaySubgraph(osg::Node* node)
{
	for(OverlayDataMap::iterator itr = _overlayDataMap.begin();
        itr != _overlayDataMap.end();
        ++itr)
    {
        ...
        if (camera)
        {
			for (int i = 0; i < camera->getNumChildren(); i++)
			{
				if (node == camera->getChild(i))
				{
