...
void
ShadowCaster1::setRanges(const std::vector<float>& ranges)
{
    _ranges = ranges;
    reinitialize();
}
...
void
ShadowCaster1::setTextureImageUnit(int unit)
{
    _texImageUnit = unit;
    reinitialize();
}
...
void
ShadowCaster1::setTextureSize(unsigned size)
{
    _size = size;
    reinitialize();
}
...
void
ShadowCaster1::setBlurFactor(float value)
{
    _blurFactor = value;
...
void
ShadowCaster1::setShadowColor(const osg::Vec4f& value)
{
    _color = value;
...
ShadowCaster1::reinitialize()
{
    ...
    std::string fragment = Stringify() << 
        ...
        "        { \n"
        "                factor = min(factor, oe_shadow_multisample(coord, c.z-0.001, oe_shadow_blur)); \n"//修改了一点
		"        } \n"
        "        else \n"
        "        { \n"
        "            float depth = texture2DArray(oe_shadow_map, coord).r; \n"
        "            if ( depth < 1.0 && depth < c.z-0.001) \n"
        "                factor = 0.0; \n"
        "        } \n"
        "    } \n"
...
void
ShadowCaster1::traverse(osg::NodeVisitor& nv)
{
    if (_supported                             && 
