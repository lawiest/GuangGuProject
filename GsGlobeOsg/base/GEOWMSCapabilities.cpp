...
namespace
{
    GEOWMSLayer* getLayerByName(const string &name, GEOWMSLayer::LayerList& layers)
    {
        for (GEOWMSLayer::LayerList::iterator i = layers.begin(); i != layers.end(); ++i)
        {
            if (osgDB::equalCaseInsensitive(i->get()->getName(),name)) return i->get();
            GEOWMSLayer *l = getLayerByName(name, i->get()->getLayers());
            if (l) return l;
        }
        return 0;
    }
...
GEOWMSStyle::GEOWMSStyle()
{
}

GEOWMSStyle::GEOWMSStyle(const std::string& name, const std::string &title)
{
    _name = name;
    _title = title;
}

GEOWMSLayer::GEOWMSLayer():
_minLon(0),
_minLat(0),
_maxLon(0),
_maxLat(0),
...
void GEOWMSLayer::getLatLonExtents(double &minLon, double &minLat, double &maxLon, double &maxLat)
{
    minLon = _minLon;
    minLat= _minLat;
    maxLon = _maxLon;
...
void GEOWMSLayer::setLatLonExtents(double minLon, double minLat, double maxLon, double maxLat)
{
    _minLon = minLon;
    _minLat = minLat;
    _maxLon = maxLon;
...
void GEOWMSLayer::getExtents(double &minX, double &minY, double &maxX, double &maxY)
{
    minX = _minX;
    minY = _minY;
    maxX = _maxX;
...
void GEOWMSLayer::setExtents(double minX, double minY, double maxX, double maxY)
{
    _minX = minX;
    _minY = minY;
    _maxX = maxX;
...
GEOWMSCapabilities::GEOWMSCapabilities()
{
}

std::string GEOWMSCapabilities::suggestExtension()
{
    //Default to png
    std::string ext = "png";
...
GEOWMSLayer*
GEOWMSCapabilities::getLayerByName(const std::string &name)
{
    return ::getLayerByName(name, _layers);
}

GEOWMSCapabilities* 
GEOWMSCapabilitiesReader::read( const std::string &location, const osgDB::ReaderWriter::Options* options )
{
    GEOWMSCapabilities *caps = NULL;
...
GEOWMSCapabilities* 
	GEOWMSCapabilitiesReader::read( const std::string &url, const std::string &location,const osgDB::ReaderWriter::Options* options )
{
	GEOWMSCapabilities *caps = NULL;
...
static void
readLayers(XmlElement* e, GEOWMSLayer* parentLayer, GEOWMSLayer::LayerList& layers)
{
    ...
    for( XmlNodeList::const_iterator i = layerNodes.begin(); i != layerNodes.end(); i++ )
    {
        ...
        GEOWMSLayer *layer = new GEOWMSLayer;
        ...
        for( XmlNodeList::const_iterator styleitr = styles.begin(); styleitr != styles.end(); styleitr++ )
        {
            ...
            layer->getStyles().push_back(GEOWMSStyle(name,title));
        }
...
GEOWMSCapabilities*
GEOWMSCapabilitiesReader::read(std::istream &in)
{
    osg::ref_ptr<GEOWMSCapabilities> capabilities = new GEOWMSCapabilities;
